...
#-------------------------------------------------------------------------------
unit class Github::App::Application:auth<github:MARTIMM>:ver<0.2.0>;
...
submethod new ( |c ) {
  ...
  self.bless(
    :GtkApplication, :app-id(github-id),
...
submethod BUILD ( *%options ) {
  ...
  my Gnome::Gio::Resource $r .= new(:load(%?RESOURCES<github.gresource>.Str));
  $r.register;
...
method local-options ( N-GObject $n-vd, :_widget($app) --> Int ) {
  ...
  my Capture $o = get-options(|$github::options-filter);
  if $o<version> {
    note "Version of github tools; $github::version";
    $exit-code = 0;
  }
...
method remote-options (
  ...
  my Capture $o = get-options-from( $args[1..*-1], |$github::options-filter);
...
method build-gui ( Gnome::Gtk3::Application :_widget($application) ) {
  ...
  my Str $required-lib = 'Github::App::MainWindow';
  require ::($required-lib);
  ::($required-lib).new(
    :$application, :app-version($github::version), :github-id(github-id)
  );
}
